...
def read_SWAT_output_sub(folder_path):   
    ...
    output_sub_df.columns = ['SUB','MON','AREA km2','PRECIP mm','SNOMELT mm','PET mm','ET mm','SW mm','PERC mm','SURQ mm',
...
def read_SWAT_output_rch(folder_path):    
    ...
    output_rch_df.columns = ['RCH','GIS','MON','AREA km2','FLOW_IN cms', 'FLOW_OUT cms','EVAP cms' ,'TLOSS cms','SED_IN tons','SED_OUT tons',
                             ...
                             'CMETAL_2 kg','CMETAL_3 kg','TOT N kg','TOT Pkg', 'NO3CONC Mg/l','WTMP degc']
    return output_rch_df,rch_num

#Basin尺度，输出年、季节的时间变化图和年内分布情况的数据
def SWAT_output_basin(folder_path,begin_year,end_year,parameters_list,out_path,flag,time_space):
    ...
    if time_space == "year":
        ...
        for par in parameters_list:
            par_y = pd.pivot_table(output_hru_year[['MON',par]],index=['MON'],values = par, aggfunc="mean")
        ...
        if flag ==1:
            ...
            for i in range(subplot_num):
                ...
                ax[i].text(year[0]+5,np.max(par_y),"y="+str(round(beta1,2))+'x'+str(round(beta0,2))+'    '+'$\it{R}$$^2$='+str(round(R2,2))+'\n'+strp+'    '+'SD='+str(round(sigma,2)),fontsize=12) 
        ...
        for par in parameters_list:
            par_mon = pd.pivot_table(output_hru_mon[['MON',par]],index=['MON'],values=par, aggfunc="mean")
            HydroParameyers_mon[par] = par_mon[par].tolist()
        ...
        for par in parameters_list:
            par_mon_y = pd.pivot_table(output_hru_mon,index=['YEAR'],columns='MON',values=[par],aggfunc=[np.mean])
        ...
        #           ax[i].plot(x, y,linestyle='-',linewidth=0.8,color=colors[j])
                    if beta0<0:
                        ax[i].text(year[0]+15,1.5*np.max(np.max(par_s))-1.5*np.max(np.max(par_s))*0.09*(j+1),"y="+str(round(beta1,2))+'x'+str(round(beta0,2))+'    '+strp,fontsize=10)
                    else:
                        ax[i].text(year[0]+15,1.5*np.max(np.max(par_s))-1.5*np.max(np.max(par_s))*0.09*(j+1),"y="+str(round(beta1,2))+'x+'+str(round(beta0,2))+'    '+strp,fontsize=10)
            plt.rcParams['font.size']=12
            plt.rcParams['font.family']='arial'
...
def SWAT_hru_space_data(folder_path,begin_year,end_year,parameters_list,out_path,flag):
    ...
    if flag ==1:
        ...
        hru_shp=floder+"/Watershed/Shapes/hru1.shp"
        ...
        for i in range(subplot_row):
            ...
            cbar2 = fig.colorbar(
                ...
                shrink=0.5,pad = 0,
                label= par+"trend mm/y"
                )
            ax1 =fig.add_axes([(0.2+0)/2, (0.25+subplot_row-1-i+0.1*i)/subplot_row, 0.1, 0.2/subplot_row])
            d1 = data[par+"_ymean"]
            d1 = d1[d1<=5*np.mean(d1)].tolist()
            sns.distplot(d1 ,ax = ax1,hist=False,fit=norm,rug=False,color="g",kde=True,
                 kde_kws={'shade':True,'color':'g'},fit_kws={'color':'r'},hist_kws={'alpha':0.6,'color':'orange'})
            plt.xlabel('') # 不添加标签
            plt.ylabel('') 
            ax2 =fig.add_axes([(0.1+1)/2, (0.25+subplot_row-1-i+0.1*i)/subplot_row, 0.1, 0.2/subplot_row])
            d2 = data[par+"_yslope"]
            d2 = d2[d2<=5*np.mean(d2)].tolist()
            sns.distplot(d2 ,ax = ax2,hist=True,fit=norm,rug=False,color="g",kde=True,
                 kde_kws={'shade':True,'color':'g'},fit_kws={'color':'r'},hist_kws={'alpha':0.6,'color':'lightgreen'})
            plt.xlabel('') # 不添加标签
            plt.ylabel('') 
        plt.rcParams['font.size']=12
        plt.rcParams['font.family']='arial'
...
def SWAT_sub_space_data(folder_path,begin_year,end_year,parameters_list,out_path,flag):
    ...
    if flag==1:
        ...
        data_geod = gp.GeoDataFrame(data)
        sub_shp = floder+"/Watershed/Shapes/sub1.shp"
        ...
        for i in range(subplot_row):
            ...
            ax1 =fig.add_axes([0.15, (0.19+subplot_row-1-i+0.25*i)/subplot_row, 0.1, 0.2/subplot_row])
            d1 = data[par+"_ymean"]
            d1 = d1[d1<=5*np.mean(d1)].tolist()
            sns.distplot(d1 ,ax = ax1,hist=False,fit=norm,rug=False,color="g",kde=True,
                 kde_kws={'shade':True,'color':'g'},fit_kws={'color':'r'},hist_kws={'alpha':0.6,'color':'orange'})
            plt.xlabel('') # 不添加标签
            plt.ylabel('') 
            ax2 =fig.add_axes([0.55, (0.19+subplot_row-1-i+0.25*i)/subplot_row, 0.1, 0.2/subplot_row])
            d2 = data[par+"_yslope"]
            d2 = d2[d2<=5*np.mean(d2)].tolist()
            sns.distplot(d2 ,ax = ax2,hist=False,fit=norm,rug=False,color="g",kde=True,
                 kde_kws={'shade':True,'color':'blue'},fit_kws={'color':'r'},hist_kws={'alpha':0.6,'color':'lightgreen'})
            plt.xlabel('') # 不添加标签
            plt.ylabel('') 
        plt.rcParams['font.size']=12
        plt.rcParams['font.family']='arial'
...
def Cent_output_basin(folder_path,begin_year,end_year,parameters_list,out_path,flag):
    ...
    if flag ==1:
        ...
        for i in range(subplot_num):
            ...
            ax[i].set(xlim=(year[0]-5,year.tolist()[-1]+5))
            ax[i].set(ylim=(0.8*np.min(par_y),1.2*np.max(par_y)))
            ax[i].text(year[0]+2,np.max(par_y),"y="+str(round(beta1,2))+'x'+str(round(beta0,2))+'    '+'$\it{R}$$^2$='+str(round(R2,2))+'\n'+strp+'    '+'SD='+str(round(sigma,2)),fontsize=12) 
            ax[i].set(ylabel = parameters_list[i])
        plt.rcParams['font.size']=12
        plt.rcParams['font.family']='arial'
        plt.rcParams['xtick.labelsize']=12
...
if __name__ == '__main__':
    folder_path = r"D:/SWAT_Daycent/SWAT_DayCent_Pro"
    begin_year = 1980
    end_year = 2019
    
    out_path = "D:/SWAT_Daycent/SWAT_DayCent_Pro/output_result/"
    flag = 1
    time_space = "year"
    #不同的函数
    parameters_list = ["PCP/mm","PET/mm","ET/mm","SW_END/mm"]
    SWAT_output_basin(folder_path,begin_year,end_year,parameters_list,out_path,flag,time_space)
#     SWAT_hru_space_data(folder_path,begin_year,end_year,parameters_list,out_path,flag)
#     SWAT_sub_space_data(folder_path,begin_year,end_year,parameters_list,out_path,flag)
#     SWAT_lulc_year(folder_path,begin_year,end_year,parameters_list,out_path)
    parameters_list = ["cproda", "aglivc"]
    Cent_output_basin(folder_path,begin_year,end_year,parameters_list,out_path,flag)
#     Cent_hru_space_data(folder_path,begin_year,end_year,parameters_list,out_path,flag)
#     Cent_sub_space_data(folder_path,begin_year,end_year,parameters_list,out_path,flag)
#     Cent_lulc_year(folder_path,begin_year,end_year,parameters_list,out_path)